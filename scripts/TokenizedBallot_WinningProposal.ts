...
async function main() {
  ...
  const WinnerVoteCount = await ballotContract.winningProposalVoteCount();
  console.log(
    `The current winning proposal is: ${ethers.decodeBytes32String(WinnerName)} - Index: ${WinnerIndex} - Voting Count: ${WinnerVoteCount}\n`
  );
}
